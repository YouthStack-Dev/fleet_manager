...
# ==========================================
# Test Cases for PUT /employee/bookings/{booking_id}
# ==========================================

class TestUpdateEmployeeBooking:
    """Test cases for updating employee bookings"""

    def test_update_booking_change_shift_success(
        self, client: TestClient, employee_token: str, 
        test_employee_booking, employee_shift_out, test_db
    ):
        """Successfully change shift for booking in Request status"""
        # Set booking date to future to avoid cutoff issues
        test_employee_booking.booking_date = date.today() + timedelta(days=5)
        test_db.commit()
        
        response = client.put(
            f"/api/v1/employee/bookings/{test_employee_booking.booking_id}",
        ...
        assert data["data"]["shift_id"] == employee_shift_out.shift_id
        assert data["data"]["status"] == "Request"

    def test_update_booking_rebook_cancelled(
        self, client: TestClient, employee_token: str, 
        test_cancelled_booking, test_db
    ):
        """Successfully re-book a cancelled booking"""
        response = client.put(
            f"/api/v1/employee/bookings/{test_cancelled_booking.booking_id}",
            json={},  # No shift change, just re-booking
            headers={"Authorization": employee_token}
        )
    ...
    def test_update_booking_rebook_and_change_shift(
        self, client: TestClient, employee_token: str, 
        test_cancelled_booking, employee_shift_out, test_db
    ):
        """Successfully re-book and change shift for cancelled booking"""
        response = client.put(
            f"/api/v1/employee/bookings/{test_cancelled_booking.booking_id}",
            json={"shift_id": employee_shift_out.shift_id},
            headers={"Authorization": employee_token}
    ...
    def test_update_booking_not_found(
        ...
        """Cannot update non-existent booking"""
        response = client.put(
            "/api/v1/employee/bookings/99999",
            json={"shift_id": 1},
            headers={"Authorization": employee_token}
        )
    ...
    def test_update_booking_invalid_status(
        self, client: TestClient, employee_token: str, 
        test_scheduled_booking, employee_shift_out
    ):
        """Cannot update booking with Scheduled status"""
        response = client.put(
            f"/api/v1/employee/bookings/{test_scheduled_booking.booking_id}",
            json={"shift_id": employee_shift_out.shift_id},
            headers={"Authorization": employee_token}
        )
        ...
        assert response.status_code == 400
        data = response.json()
        assert "status" in str(data).lower() or "request" in str(data).lower() or "cancelled" in str(data).lower()

    def test_update_booking_invalid_shift(
        self, client: TestClient, employee_token: str, test_employee_booking
    ):
        """Cannot update booking with invalid shift_id"""
        response = client.put(
            f"/api/v1/employee/bookings/{test_employee_booking.booking_id}",
            json={"shift_id": 99999},
            headers={"Authorization": employee_token}
        ...
        assert response.status_code == 400
        data = response.json()
        assert "shift" in str(data).lower() or "invalid" in str(data).lower()

    def test_update_booking_duplicate_shift(
        self, client: TestClient, employee_token: str, 
        test_db, test_tenant, employee_shift_in, test_employee_app,
        employee_shift_out
    ):
        """Cannot update to shift that already has a booking on same date"""
        # Create first booking for shift_out
        booking1 = Booking(
            tenant_id=test_tenant.tenant_id,
            employee_id=test_employee_app.employee_id,
            employee_code=test_employee_app.employee_code,
            shift_id=employee_shift_out.shift_id,
            team_id=test_employee_app.team_id,
        ...
        # Create second booking for shift_in
        booking2 = Booking(
            tenant_id=test_tenant.tenant_id,
            employee_id=test_employee_app.employee_id,
            employee_code=test_employee_app.employee_code,
        ...
        # Try to update booking2 to shift_out (duplicate)
        response = client.put(
            f"/api/v1/employee/bookings/{booking2.booking_id}",
            json={"shift_id": employee_shift_out.shift_id},
            headers={"Authorization": employee_token}
        )
    ...
    def test_update_booking_past_shift(
        self, client: TestClient, employee_token: str, 
        test_db, test_tenant, test_employee_app, employee_shift_in
    ):
        """Cannot update to a shift that has already passed"""
        ...
        # Create a past shift (early morning)
        past_shift = Shift(
            tenant_id=test_tenant.tenant_id,
            shift_code="EARLY_MORNING",
            shift_time=time(6, 0, 0),
            log_type=ShiftLogTypeEnum.IN,
            is_active=True
        )
        test_db.add(past_shift)
        test_db.commit()
        test_db.refresh(past_shift)
        
        # Try to update to past shift (if current time is after 6 AM)
        current_time = datetime.now().time()
        if current_time > time(6, 0, 0):
            response = client.put(
                f"/api/v1/employee/bookings/{booking.booking_id}",
                json={"shift_id": past_shift.shift_id},
                headers={"Authorization": employee_token}
            )
            
            # Should fail with past shift error
            assert response.status_code in [200, 400]  # May succeed if before 6 AM

    def test_update_booking_with_cutoff_validation(
        self, client: TestClient, employee_token: str, 
        test_db, test_tenant, test_employee_app, employee_shift_in, test_cutoff
    ):
        """Cutoff validation applies when updating shift"""
        # Create booking for today with near-future shift
        booking = Booking(
            tenant_id=test_tenant.tenant_id,
            employee_id=test_employee_app.employee_id,
            employee_code=test_employee_app.employee_code,
        ...
        # Create a shift very soon (within cutoff window)
        near_future_time = (datetime.now() + timedelta(hours=1)).time()
        near_shift = Shift(
            tenant_id=test_tenant.tenant_id,
            shift_code="NEAR_SHIFT",
            shift_time=near_future_time,
            log_type=ShiftLogTypeEnum.IN,
            is_active=True
        )
        test_db.add(near_shift)
        test_db.commit()
        test_db.refresh(near_shift)
        
        # Try to update to near shift (should fail due to cutoff)
        response = client.put(
            f"/api/v1/employee/bookings/{booking.booking_id}",
            json={"shift_id": near_shift.shift_id},
            headers={"Authorization": employee_token}
        )
        
        # Should fail with cutoff error (cutoff is 2 hours)
        assert response.status_code == 400
        data = response.json()
        assert "cutoff" in str(data).lower()

    def test_update_booking_unauthorized(self, client: TestClient):
        """Cannot update booking without authentication"""
        response = client.put(
            "/api/v1/employee/bookings/1",
            json={"shift_id": 2}
        )
        
        assert response.status_code == 401

    def test_update_booking_different_employee(
        self, client: TestClient, test_db, test_tenant, 
        test_team, employee_shift_in
    ):
        """Cannot update another employee's booking"""
        ...
        test_db.commit()
        test_db.refresh(other_employee)
        
        # Create booking for other employee
        booking = Booking(
            tenant_id=test_tenant.tenant_id,
            employee_id=other_employee.employee_id,
            employee_code=other_employee.employee_code,
            shift_id=employee_shift_in.shift_id,
            team_id=test_team.team_id,
            booking_date=date.today() + timedelta(days=1),
            booking_type=BookingTypeEnum.REGULAR,
        ...
        # Create token for first employee
        from common_utils.auth.utils import create_access_token
        token = create_access_token(
            user_id="200",  # First employee
            tenant_id=test_tenant.tenant_id,
            user_type="employee",
            custom_claims={
                "permissions": [
        ...
        # Try to update other employee's booking
        response = client.put(
            f"/api/v1/employee/bookings/{booking.booking_id}",
        ...
        assert response.status_code == 404  # Not found because it belongs to different employee
...
class TestEmployeeBookingFlow:
    """Integration test covering complete employee booking workflow"""
